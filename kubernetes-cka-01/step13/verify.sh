     ...
     then
       ...
       if [[ "$prenom" != "CHANGEZ-MOI" && "$nom" != "CHANGEZ-MOI" && "$code" != "CHANGEZ-MOI"  && "$email" != "CHANGEZ-MOI" ]]; then
          ...
          curl --fail -X POST -H 'Content-Type: application/json' \
            ...
            \"attachments\":[{\"title\":\"$msg du Challenge cka-002 (killercoda) avec un score de $p%\",
            \"title_link\":\"https://killercoda.com/hleclerc/scenario/kubernetes-cka-02\",\"text\":\"$prenom $nom ($email) a passé la certification cka-001 avec un résultat de $p% ($msg)\",\"color\":\"#764FA5\"}]}" \
